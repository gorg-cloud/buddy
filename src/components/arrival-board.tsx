...
export function ArrivalBoard() {
  ...
  // Real arrivals, straight from the database.
  useEffect(() => {
    let cancelled = false;
    fetch("/api/arrivals")
            ...
            <div className="divide-y divide-amber/10">
              {loading ? (
                Array.from({ length: VISIBLE }, (_, i) => (
                  <div
                    key={i}
                    className="board grid grid-cols-[5.2rem_1fr_1fr_auto] items-center gap-3 px-4 py-3"
                    aria-hidden
                  >
                    <span className="h-4 w-16 animate-pulse bg-amber/20" />
                    <span className="h-4 w-20 animate-pulse bg-paper/15" />
                    <span className="h-4 w-24 animate-pulse bg-paper/15" />
                    <span className="h-3 w-12 animate-pulse bg-paper/15" />
                  </div>
                ))
              ) : rows.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-2 px-4 py-9 text-center">
                  <span className="board inline-flex items-center gap-2 text-[12px] tracking-[0.25em] text-amber">
                    <span className="size-1.5 animate-pulse rounded-full bg-amber shadow-[0_0_7px_1px_rgba(242,165,22,0.6)]" />
                    No arrivals yet
                  </span>
                  <span className="board text-[10px] tracking-[0.2em] text-paper/45">
                    Be the first to land — your flight appears here the moment
                    you sign up.
                  </span>
                </div>
              ) : (
